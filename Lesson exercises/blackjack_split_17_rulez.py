...
print("Dealer draws at 16 and stops after 17")
time.sleep(1)
# blackjack condition
if h1==None:
...
if h1==None:
	...
	if ans=="s":
		...
		while (totald<total):
			if (totald>=17):
				print("Dealer has to stop because he has reached 17 limit")
				time.sleep(1)
				break
			else:
	...
	if ans2=="s" or ans1=="s":
		...
		print("Dealer has a total of",totald)
		# d3 draw
		while (totald<total1) or (totald<total2):
			if (totald>=17):
				print("Dealer has to stop because he has reached 17 limit")
				time.sleep(1)
				break
		...
		if (totald==total1) and (totald==total2):
			...
			print("Dealer wins against second pair but loses to the first")
			k=input("Press enter to exit")
			exit()
		else:
